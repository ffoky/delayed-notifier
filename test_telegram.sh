...
TELEGRAM_ID=993309493
DELAY_MINUTES=5

echo "Создаём пользователя с telegram_id: $TELEGRAM_ID"
...
NOTIFY_RESPONSE=$(curl -s -X POST "$BASE_URL/notify" \
  -H "Content-Type: application/json" \
  -d "{\"user_id\": \"$USER_ID\", \"channel\": \"telegram\", \"text\": \"Тест Telegram уведомления!\", \"planned_at\": \"$PLANNED_AT\"}")
...
echo "Сообщение придёт в Telegram через $DELAY_MINUTES мин"